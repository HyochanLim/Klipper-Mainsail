...
export const getters: GetterTree<GuiState, RootState> = {
    ...
    getAllPossiblePanels: (state, getters, rootState, rootGetters) => {
        ...
        const cmykwAxes = ['A', 'B', 'C', 'D', 'H']
        const axisMap = rootState.printer?.gcode_move?.axis_map ?? {}
        const commands = rootState.printer?.gcode?.commands ?? {}
        const hasCmykwAxes = cmykwAxes.some((axis) => axis in axisMap)
        const hasCmykwRegisterMacro = 'REGISTER_CMYKW_AXES' in commands
        if (!hasCmykwAxes && !hasCmykwRegisterMacro) {
            allPanels = allPanels.filter((name) => name !== 'filament-mixing')
        }
